...
def main_worker(gpu, ngpus_per_node, args):
    ...
    train_loader = data.DataLoader(
        ...
        num_workers=args.num_workers,  # 先用 2
        shuffle=True,
        drop_last=True
    )
...
def gen_plot(gen_net, epoch, num_classes=10, latent_dim=128):
    ...
    fig.suptitle(f'Synthetic spectrograms at epoch {epoch}', fontsize=20)
    ...
    for i in range(rows):
        for j in range(cols):
            ...
            with torch.no_grad():
                fake = gen_net(z, y).cpu().numpy()  # 期望 [B, C, F] 或 [B, 1, C, F]

            # --- 兼容两种常见输出形状 ---
            if fake.ndim == 4:     # [B, 1, C, F]
                spec = fake[0, 0]  # -> [C, F]
            elif fake.ndim == 3:   # [B, C, F]
                spec = fake[0]     # -> [C, F]
            else:
                raise RuntimeError(f"Unexpected fake shape from Generator: {fake.shape}")

            axs[i, j].imshow(spec, aspect='auto', origin='lower')
            axs[i, j].set_title(f"label={int(y)}")
            axs[i, j].set_xlabel("Freq bins")
            axs[i, j].set_ylabel("Channels")
            k += 1

    buf = io.BytesIO()
    plt.tight_layout()
    plt.savefig(buf, format='jpeg', dpi=120)
    buf.seek(0)
    return buf
